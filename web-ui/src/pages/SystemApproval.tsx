...
interface PhaseStatus {
  uiAssets: { total: number; approved: number; rejected: number; items: SystemItem[] };
  uiFramework: { total: number; approved: number; rejected: number; items: SystemItem[] };
  uiStyles: { total: number; approved: number; rejected: number; items: SystemItem[] };
  uiDesigner: { total: number; approved: number; rejected: number; items: SystemItem[] };
}
...
const defaultSystemItems: SystemItem[] = [
  {
    id: 'ui-assets-review',
...
export const SystemApproval: React.FC = () => {
  ...
  const loadSystemItems = async () => {
    ...
    try {
      ...
      if (currentWorkspace?.projectFolder) {
        try {
          const response = await fetch(`${INTEGRATION_URL}/read-file`, {
            ...
            body: JSON.stringify({
              filePath: `${currentWorkspace.projectFolder}/approvals/system-approvals.json`
            }),
          });
      ...
      if (Object.keys(approvals).length === 0) {
        const stored = localStorage.getItem(`system-item-approvals-${currentWorkspace.id}`);
      ...
      const itemsWithStatus = defaultSystemItems.map(item => {
      ...
      try {
        ...
        await fetch(`${INTEGRATION_URL}/save-specifications`, {
          ...
          body: JSON.stringify({
            ...
            specifications: [{
              filename: 'approvals/system-approvals.json',
              content: fileContent,
            }],
          }),
        });
  ...
  const handleChecklistChange = (item: SystemItem, checklistId: string, checked: boolean) => {
  ...
  const handleApproveItem = (item: SystemItem) => {
    const newApprovals = {
      ...itemApprovals,
      [item.id]: {
  ...
  const handleRejectItem = (item: SystemItem) => {
  ...
  const handleResetItemStatus = (item: SystemItem) => {
  ...
  const renderSystemItemCard = (item: SystemItem) => {
  ...
  return (
    <PageLayout
      ...
      quickDescription="Review and approve all system phase items before proceeding."
      detailedDescription="The System phase includes UI Assets, UI Framework selection, UI Styles, and UI Designer outputs.
...
Complete all checklist items and approve each section before proceeding to Implementation."
    ...
    >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
          ...
          <div style={{
            ...
            background: `conic-gradient(
              ${systemApproved ? 'var(--color-systemGreen)' : getTotalRejected() > 0 ? 'var(--color-systemRed)' : 'var(--color-systemBlue)'} ${getCompletionPercentage()}%,
      ...
      {/* Requirements Info */}
      {!canApprovePhase() && !systemApproved && (
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '40px' }}>
          <p className="text-body text-secondary">Loading system items...</p>
        </div>
      ) : (
        <>
          {/* System Item Cards */}
          {allItems.map(item => renderSystemItemCard(item))}
        </>
      )}

      {/* INTENT Info */}
      <Alert type="info" style={{ marginTop: '24px' }}>
        <strong>INTENT System Phase:</strong> The system phase establishes how your application will look and feel.

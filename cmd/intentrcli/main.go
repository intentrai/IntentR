...
import (
	...
	"github.com/jareynolds/intentr/cmd/intentrcli/adapters"
	"github.com/jareynolds/intentr/cmd/intentrcli/cli"
	"github.com/jareynolds/intentr/cmd/intentrcli/config"
	"github.com/jareynolds/intentr/cmd/intentrcli/pipeline"

	// Import adapters to trigger init() registration
	_ "github.com/jareynolds/intentr/cmd/intentrcli/adapters"
)
...
func main() {
	...
	// Handle config init
	if *configInit {
		if err := config.InitDefaultConfig(".intentrcli.yaml"); err != nil {
			fmt.Fprintf(os.Stderr, "Error: %v\n", err)
			os.Exit(1)
		}
		fmt.Println("Created .intentrcli.yaml with default configuration")
		os.Exit(0)
	}
...
USAGE:
  intentrcli [OPTIONS]
  intentrcli -p "prompt"
  echo "prompt" | intentrcli

OPTIONS:
  -version        Show version information
  -help           Show this help message
  -init           Create default .intentrcli.yaml configuration
...
CONFIGURATION:
  Global:  ~/.intentrcli.yaml
  Project: ./.intentrcli.yaml
...
EXAMPLES:
  ...
  intentrcli -adapter openai

  # Execute single prompt
  intentrcli -p "Explain this code"

  # Pipe input
  cat prompt.txt | intentrcli

  # Initialize configuration
  intentrcli -init

For more information: https://github.com/intentr/intentrcli
`, version)
}

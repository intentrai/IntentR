...
| **Approval** | Approved |
| **Priority** | High |
| **Implementation Location** | `cmd/intentrcli/` |
...
- Configuration file support (.intentrcli.yaml)
...
|------------|------|---------|--------|----------------|
| ENB-759321 | UbeCLI Interactive Shell | Provides Claude CLI-like interactive experience with REPL, multi-line input, slash commands | Implemented | `cmd/intentrcli/cli/shell.go` |
| ENB-759322 | Prework Pipeline | Orchestrates 5-stage prompt transformation (context, rules, approval, build, format) | Implemented | `cmd/intentrcli/pipeline/pipeline.go` |
| ENB-759323 | Context Loader | Loads project files, specifications, with caching and auto-detection | Implemented | `cmd/intentrcli/pipeline/context_loader.go` |
| ENB-759324 | LLM Adapter Interface | Pluggable adapter registry with interface for any LLM backend | Implemented | `cmd/intentrcli/adapters/adapter.go` |
| ENB-759325 | Configuration Management | Hierarchical YAML config with env overrides | Implemented | `cmd/intentrcli/config/config.go` |
| ENB-759326 | Global Installation | Makefile targets for system-wide PATH installation | Implemented | `Makefile` |
| ENB-759327 | Codex CLI Adapter | OpenAI Codex CLI integration adapter with exec mode | Implemented | `cmd/intentrcli/adapters/codex_cli.go` |
...
| .intentrcli.yaml | Configuration | User configuration file |
...
| FR-759008 | Configuration File | System reads .intentrcli.yaml for settings | Ready for Design | Medium | Approved |
...
```mermaid
flowchart LR
    subgraph UbeCLI["cmd/intentrcli"]
        MAIN[main.go]
        SHELL[cli/shell.go]
...
### Configuration File Format (.intentrcli.yaml)
...
shell:
  prompt: 'ube> '
  history_file: ~/.intentrcli_history
  max_history: 1000

# Logging
logging:
  enabled: true
  file: ~/.intentrcli/logs/audit.log
  level: info
...
2. Global config: `~/.intentrcli.yaml`
3. Project config: `./.intentrcli.yaml`
...
- **CLI Binary**: `cmd/intentrcli/`
- **Configuration**: `~/.intentrcli.yaml` or `./.intentrcli.yaml`
- **History**: `~/.intentrcli_history`
- **Logs**: `~/.intentrcli/logs/`

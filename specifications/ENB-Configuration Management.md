...
2. **File Format**: YAML configuration files (.intentrcli.yaml)
...
|----|------|-------------|--------|----------|----------|
| FR-759501 | YAML Loading | Config loads from .intentrcli.yaml files | Implemented | High | Approved |
...
flowchart TD
    subgraph Sources["Configuration Sources (Priority Order)"]
        ...
        GLOBAL[2. Global ~/.intentrcli.yaml]
        PROJECT[3. Project ./.intentrcli.yaml]
...
func (m *ConfigManager) Load() error {
    ...
    // 2. Load global config (~/.intentrcli.yaml)
    globalPath := filepath.Join(os.Getenv("HOME"), ".intentrcli.yaml")
    if globalConfig, err := loadYAML(globalPath); err == nil {
        config = mergeConfig(config, globalConfig)
    }

    // 3. Load project config (./.intentrcli.yaml)
    projectPath := ".intentrcli.yaml"
...
func DefaultConfig() *Config {
    return &Config{
        ...
        Shell: ShellConfig{
            Prompt:      "ube> ",
            HistoryFile: "~/.intentrcli_history",
            MaxHistory:  1000,
        },
...
```yaml
# ~/.intentrcli.yaml or ./.intentrcli.yaml
...
Feature: Configuration Management
  ...
  Scenario: Load YAML configuration
    Given .intentrcli.yaml exists with valid content
  ...
  Scenario: Merge configuration hierarchy
    Given ~/.intentrcli.yaml sets llm.default to "claude-api"
    And ./.intentrcli.yaml sets llm.default to "ollama"
    When the config manager loads configuration
    Then llm.default should be "ollama"

  @TS-759503 @FR-759503 @critical
  Scenario: Environment variable override
    Given .intentrcli.yaml sets llm.default to "claude-cli"
  ...
  Scenario: Reject invalid configuration
    Given .intentrcli.yaml has invalid timeout value
    When the config manager loads configuration
  ...
  Scenario: Initialize default config file
    Given no .intentrcli.yaml exists
    When I run "/config init"
    Then .intentrcli.yaml should be created

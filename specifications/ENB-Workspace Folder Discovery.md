...
| FR-931202 | Config Detection | System detects presence of .intentrworkspace in each folder | Implemented | High | Approved |
...
|-----------|-------|---------|
| "Has .intentrworkspace" | Green badge | Folder contains config file |
...
Feature: Workspace Folder Discovery
  ...
  Scenario: Display config status
    Given folder "ConfiguredProject" has a .intentrworkspace file
    And folder "UnconfiguredProject" does not have a .intentrworkspace file
    When I scan workspace folders
    Then "ConfiguredProject" should show "Has .intentrworkspace" badge
    And "UnconfiguredProject" should show "No config file" badge

  @TS-931203 @FR-931205
  Scenario: Import with config
    Given discovered folder "ProjectX" has a .intentrworkspace with name "Project X"
  ...
  Scenario: Import without config
    Given discovered folder "LegacyProject" has no .intentrworkspace file
    When I click "Import Workspace" for "LegacyProject"
    Then a workspace named "LegacyProject" should be added to my list
    And a new .intentrworkspace file should be created
